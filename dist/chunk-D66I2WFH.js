import { useChatContext, useConversationHistory, useChat, useIsMobile, useVisualViewport, useKeyboardOpen, overlayHeight, getSlashCommandRegistry, extractSuggestionsFromContent, extractArtifactsFromContent, useStickyBottom, useArtifacts, useSources, useSessionFiles, ChatStateProvider, useViewportHeightFallback } from './chunk-6RQRE6TT.js';
import { defaultStrings } from './chunk-XEHEI7WA.js';
...
function CommandPalette() {
  ...
  return /* @__PURE__ */ jsx(DialogPrimitive.Root, { onOpenChange: setOpen, open, children: /* @__PURE__ */ jsx(
    DialogPrimitive.Portal,
    {
      container: typeof document !== "undefined" ? document.documentElement : void 0,
      children: /* @__PURE__ */ jsxs("div", { "data-chat-provider": "ai-chat-sdk", "data-theme": config.theme, children: [
        /* @__PURE__ */ jsx(DialogPrimitive.Overlay, { className: "ais-dialog-overlay" }),
        /* @__PURE__ */ jsxs(DialogPrimitive.Content, { className: paletteClass, style: sheetStyle, children: [
          ...
          !isMobile ? /* @__PURE__ */ jsxs("div", { className: "ais-command-footer", children: [
...
function ChatSidebar({
  ...
  const preAutoCollapseRef = React16.useRef(false);
  const userToggledDuringArtifactRef = React16.useRef(false);
...
function ChatShellContent({
  ...
  useEffect(() => {
    ...
    if (prevStreamingRef.current.isStreaming === isStreaming && prevStreamingRef.current.resumeState === resumeState)
      return;
...
function ChatSidepanelLayout({
  ...
  useEffect(() => {
    if (typeof window === "undefined") return;
    const onResize = () => {
      const next = window.innerWidth <= SIDEBAR_OVERLAY_BREAKPOINT_PX3;
      setIsOverlayViewport((prev) => prev === next ? prev : next);
    };
    window.addEventListener("resize", onResize);
    return () => window.removeEventListener("resize", onResize);
  }, []);
  const savedLayoutRef = useRef(getSafeLayout(storageKey));
  useEffect(() => {
  ...
  const overlayPaneStyle = keyboardOpen && viewport ? {
  ...
  return /* @__PURE__ */ jsxs(
    "div",
    {
      className: `ais-sidepanel-layout-root ${isOverlayViewport ? "ais-mobile" : "ais-desktop"} ${isOpen ? "ais-panel-open" : ""} ${keyboardOpen ? "is-keyboard-open" : ""} ${className}`,
...
//# sourceMappingURL=chunk-D66I2WFH.js.map
//# sourceMappingURL=chunk-D66I2WFH.js.map